...
test_that("session info is omitted / included", {
  ...
  ret <- reprex(input = input, render = FALSE, template = TRUE)
  expect_false(any(grepl(regex, ret)))
  ret <- reprex(input = input, si = TRUE, render = FALSE, template = TRUE)
  expect_match(ret, regex, fixed = TRUE, all = FALSE)
  ret <- reprex(
    input = input, venue = "so", si = TRUE, render = FALSE, template = TRUE
  )
  expect_match(ret, regex, fixed = TRUE, all = FALSE)
})
...
test_that("session info is folded on github", {
  ...
  input <- c("(y <- 1:4)", "mean(y)")
  ret <- reprex(
    input = input, render = FALSE, si = TRUE, venue = "gh", template = TRUE
  )
  expect_match(
