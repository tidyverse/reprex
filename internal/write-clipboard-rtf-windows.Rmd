...
It looked something like this, which I save to `write_clipboard_rtf_beta.ps1`:
...
In addition to being more convoluated, this had a huge functional problem: the RTF disappears from the clipboard as soon as the PowerShell exits.
...
But if I ever need the more complicated version -- maybe to write an image? -- I'd have to grapple with this.
I think there might be a way to request persistence FILL THIS IN.
This would also go away if we were accessing the clipboard more directly.
