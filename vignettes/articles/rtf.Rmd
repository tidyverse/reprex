...
Use `reprex_rtf()` or `reprex(venue = "rtf")` to get output in [Rich Text Format](https://en.wikipedia.org/wiki/Rich_Text_Format).
This is useful for creating syntax highlighted code snippets -- rendered or not -- to paste into software such as Keynote or PowerPoint.
...
For macOS users who use [homebrew](https://brew.sh):

```{sh eval = FALSE}
brew install highlight
```

For Windows users who use [chocolatey](https://chocolatey.org/):

```{sh eval = FALSE}
choco install highlight
```

On Windows, this does not seem to add highlight to your `PATH`.
You may [need to do this manually](https://www.maketecheasier.com/what-is-the-windows-path/).
In my case, I added `C:\Program Files\Highlight` to `PATH`.
...
Once, for an important talk, I even created a custom highlight theme that matched my Keynote theme (see [how-to notes](https://github.com/jennybc/debugging/blob/master/how-to/highlight-custom-theme.md)).
